...
export class CatalogRepository {
  ...
  async getMasterItemById(id: string) {
    return supabase
      .from("master_catalog")
      .select("id, name, category, default_price, grade_from, grade_to")
      .eq("id", id)
  ...
  async findInstitutionBySlug(slug: string) {
    return supabase
      .from("institutions")
      .select(
        "id, name, slug, form_slug, email, mobile, status, form_start_date, form_end_date, payment_methods, online_payment_instruction"
      )
      .or(`slug.eq.${slug},form_slug.eq.${slug}`)
      .maybeSingle();
  }

  async listInstitutionItemsByClass(institutionId: string, className?: string) {
    const itemsQuery = supabase
      .from("institution_items")
      .select(
        "id, institution_id, master_item_id, name, category, price, is_active, master_catalog(grade_from, grade_to)"
      )
      .eq("institution_id", institutionId)
      .eq("is_active", true)
      .order("name", { ascending: true });
  ...
  async listMasterItemsByClass(className?: string) {
